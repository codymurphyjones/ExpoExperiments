...
const UserRegistration = (props: UserInformationProps) => {
  ...
  return (	
    <View style={style.container}>
              ...
              <IconTextBox ref={passwordBox} onSubmit={() => { confirmpasswordBox.current.focus()}} borderColor={(!isPass  && password.length < 6) ? "red" : "#bbb"} onChangeText={text => { setIsPass(passwordValidate(text,confirm)); setIsConfirm(text === confirm); setPassword(text); }}width="100%" icon="lock" placeholder="Password" password={true}  />
              <IconTextBox ref={confirmpasswordBox} onSubmit={() => {}}  borderColor={(!isConfirm || confirm.length < 6) ? "red" : "#bbb"} onChangeText={text => {  setConfirm(text); setIsPass(passwordValidate(password,text)); setIsConfirm(password === text); }} icon="shield" placeholder="Confirm password" password={true} width="100%" />
